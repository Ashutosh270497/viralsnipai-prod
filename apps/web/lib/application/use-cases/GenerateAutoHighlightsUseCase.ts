...
import {
  ClipCandidateGenerationService,
  type ClipCandidate,
} from "@/lib/domain/services/ClipCandidateGenerationService";
...
import {
  ...
  type TranscriptEnhancement,
} from "@/lib/services/transcript-enhancement.service";
import { logger } from "@/lib/logger";
import { AppError } from "@/lib/utils/error-handler";
import type { Clip, ViralityFactors as ClipViralityFactors, EnhancementData, VideoGeometry } from "@/lib/types";
...
export class GenerateAutoHighlightsUseCase {
  ...
  async execute(input: GenerateAutoHighlightsInput): Promise<GenerateAutoHighlightsOutput> {
    ...
    const defaultBrandTemplate = await this.getDefaultBrandTemplate(userId);
    const brandDefaults = buildNewClipBrandDefaults(defaultBrandTemplate);
    ...
    // this streams the file to a tempfile.
    ...
    try {
      ...
      const serializedTranscript =
        this.transcriptionService.serializeTranscription(transcriptionResult);
      ...
      let sourceGeometry: VideoGeometry | null =
          ...
          ? {
              ...
              sourceRatioLabel:
                Math.abs(asset.sourceWidth / asset.sourceHeight - 9 / 16) < 0.05
                  ...
                  : Math.abs(asset.sourceWidth / asset.sourceHeight - 1) < 0.05
                    ? ("1:1" as const)
                    : Math.abs(asset.sourceWidth / asset.sourceHeight - 4 / 5) < 0.05
                      ? ("4:5" as const)
      ...
      try {
        ...
        logger.warn(
          "OpenRouter candidate reranking failed; falling back to deterministic candidate order",
          {
            assetId,
            error: error instanceof Error ? error.message : String(error),
          },
      ...
      const extractedClips = selectedCandidatePairs.map((pair) => {
        ...
        const qualitySignals = analyzeClipQuality({
          ...
          sceneCutsMs,
          minDurationMs: clipPolicy.minMs,
          maxDurationMs: clipPolicy.maxMs,
        });
      ...
      if (mode === "replace") {
        ...
        const filtered = extractedClips.filter((candidate) => {
          const collides = existingStartsMs.some(
            (existingStart) =>
              Math.abs(existingStart - candidate.startMs) <= MERGE_OVERLAP_WINDOW_MS,
          );
        ...
        if (filtered.length === 0) {
          logger.warn(
            "Auto-highlights merge mode: every candidate overlapped with existing clips",
            {
              assetId,
              projectId: asset.projectId,
            },
      ...
      if (transcriptionResult.segments && transcriptionResult.segments.length > 0) {
        ...
        for (let i = 0; i < clipsToCreate.length; i++) {
          ...
          if (segments.length > 0) {
            try {
              const enhancement: TranscriptEnhancement =
                await transcriptEnhancementService.analyzeTranscript(
                  segments,
      ...
      for (let i = 0; i < clipsToCreate.length; i++) {
        ...
        const blendedScore = blendViralityScore(
          viralityAnalysis?.score,
          qualitySignals.overallScore,
        );
        ...
        const clip = await this.clipRepo.create({
          ...
          callToAction: extractedClip.callToAction ?? brandDefaults.callToAction,
          viralityScore: blendedScore,
          viralityFactors,
          ...(brandDefaults.captionStyle ? { captionStyle: brandDefaults.captionStyle } : {}),
      ...
      return {
        ...
        analytics: {
          ...
          lowPrecisionWarning:
            getLowPrecisionWarning(canonicalTranscript) ?? rerankWarnings[0] ?? null,
          clipLengthPreset,
          clipPolicy: {
            minMs: clipPolicy.minMs,
            idealMs: clipPolicy.idealMs,
  ...
  private buildClipCaptionSrt(
    ...
    if (dedupedWords.length > 0) {
      ...
      for (let i = 0; i < dedupedWords.length; i += wordsPerCue) {
        ...
        const endMs = Math.max(startMs + 120, Math.round((last.end - clipStartSec) * 1000));
        const text = chunk
          .map((word) => word.word)
          .join(" ")
          .trim();
  ...
  private async getDefaultBrandTemplate(userId: string): Promise<BrandTemplateRecord | null> {
    try {
      const row = await (prisma as any).brandTemplate.findFirst({
        where: { userId, isDefault: true },
      });
      return row ? serializeBrandTemplate(row) : null;
    } catch (error) {
      logger.warn("Unable to load default brand template for auto-highlights", {
        userId,
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
