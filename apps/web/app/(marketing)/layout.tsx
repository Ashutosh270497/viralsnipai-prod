...
const legacyLinks = [
  { href: "/signin", label: "Sign in", variant: "ghost" as const },
  { href: "/signin", label: "Get started", variant: "default" as const }
];
...
export default function MarketingLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      {uiV2 ? <MarketingHeader /> : <LegacyHeader />}
      <main id="main" className="flex-1">
        {children}
      </main>
      <Footer />
      {uiV2 ? <CookieBanner /> : null}
    </div>
  );
}
...
function LegacyHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 px-6 py-4 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          {legacyLinks.map((link) => (
            <Button key={link.label} variant={link.variant} asChild>
  ...
  return (
    <footer className="border-t border-border/60 bg-background/90 px-6 py-10 text-xs text-muted-foreground">
      <div className="mx-auto grid w-full max-w-6xl gap-6 text-left sm:grid-cols-2 lg:grid-cols-4">
        <FooterColumn title="Product" links={[{ href: "#features", label: "Features" }, { href: "#templates", label: "Templates" }, { href: "/pricing", label: "Pricing" }]} />
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col items-start justify-between gap-4 pt-8 text-muted-foreground sm:flex-row sm:items-center">
        <p>Clippers © {new Date().getFullYear()} • Built for video-first teams. Clippers complies with platform API policies. No scraping.</p>
