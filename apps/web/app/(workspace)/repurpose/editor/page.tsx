...
export default function RepurposeEditorPage() {
  ...
  const [sortBy, setSortBy] = useState<"score" | "confidence" | "duration">("score");
  const [hideLowConfidence, setHideLowConfidence] = useState(false);
  const [platformFilter, setPlatformFilter] = useState<"all" | "youtubeShorts" | "instagramReels" | "tiktok" | "x">("all");
  const [reviewStatusByClip, setReviewStatusByClip] = useState<Record<string, "needs_review" | "approved" | "rejected" | "export_ready">>({});
  ...
  const visibleClips = useMemo(() => {
    ...
    return [...clips]
      .filter((clip) => {
        ...
        return reviewStatusByClip[clip.id] !== "rejected";
      })
      .sort((a, b) => {
        if (sortBy === "duration") return (b.endMs - b.startMs) - (a.endMs - a.startMs);
        if (sortBy === "confidence") {
          return (confidenceRank[getClipMetadata(b).boundaryConfidence ?? "low"] ?? 0) -
            (confidenceRank[getClipMetadata(a).boundaryConfidence ?? "low"] ?? 0);
        }
        return (b.viralityScore ?? 0) - (a.viralityScore ?? 0);
      });
  }, [clips, hideLowConfidence, platformFilter, reviewStatusByClip, sortBy]);
  ...
  const activeReviewStatus =
    activeClip
      ? reviewStatusByClip[activeClip.id] ?? (selectedClipIds.includes(activeClip.id) ? "export_ready" : "needs_review")
      : "needs_review";
  ...
  function setReviewStatus(id: string, status: "needs_review" | "approved" | "rejected" | "export_ready") {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-emerald-400/75">AI clip review queue</p>
          <h2 className="mt-1 text-2xl font-bold">Edit & Enhance</h2>
          <p className="text-sm text-muted-foreground mt-0.5">
            Preview clips, inspect confidence signals, edit transcript timing, and approve export-ready shorts.
          </p>
        </div>
      ...
      <div className="grid gap-3 rounded-2xl border border-border/60 bg-card/50 p-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          {(["needs_review", "approved", "rejected", "export_ready"] as const).map((status) => (
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-[calc(100vh-260px)] space-y-1.5 overflow-y-auto rounded-2xl border border-border/40 bg-card/30 p-2">
            {visibleClips.map((clip, index) => {
              ...
              const metadata = getClipMetadata(clip);
              const reviewStatus = reviewStatusByClip[clip.id] ?? (isSelected ? "export_ready" : "needs_review");
              ...
              <div className="grid gap-3 border-b border-border/40 px-5 py-4 md:grid-cols-[minmax(0,1fr)_260px]">
                <div className="rounded-xl border border-border/50 bg-muted/25 p-3">
                  ...
                  <p className="mt-2 text-sm leading-6 text-muted-foreground">
                    {activeMetadata.viralReason || activeClip.viralityFactors?.reasoning || "Selected by deterministic transcript quality, hook density, pacing, and OpenRouter ranking."}
                  </p>
                </div>
                <div className="rounded-xl border border-border/50 bg-muted/25 p-3">
