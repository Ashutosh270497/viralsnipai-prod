# ViralSnipAI Web Application - Architecture Documentation

## Overview

This document describes the architecture of the ViralSnipAI web application after the Phase 1 & 2 refactoring implementing SOLID principles and Clean Architecture.
...
FFmpeg-backed media work currently uses a transitional execution model:

- FFmpeg and ffprobe are resolved from `ffmpeg-static`, `ffprobe-static`, or explicit environment overrides.
- Jobs are orchestrated through `@clippers/jobs`.
...
This is acceptable for local development and transitional deployment, but it should not be treated as the long-term production runtime for heavy media workloads.
...
- explicit retries and isolated resource limits away from the main web runtime

### Failure Model

The documented failure classes for FFmpeg-backed jobs are:

- missing FFmpeg or ffprobe binary
- missing or stale source asset path
- output write or permissions failure
- queue stall or worker unavailability

Jobs should fail explicitly and record actionable error states rather than silently hanging.
...
The repo now treats experience standards as explicit source-of-truth artifacts, not implicit expectations.

- **Responsive support matrix:** `apps/web/lib/platform/responsive-support-matrix.ts`
  - distinguishes `mobile_supported`, `mobile_degraded`, and `desktop_only`
  - keeps RepurposeOS editor/export explicitly desktop-only for now
- **Accessibility baseline:** `apps/web/lib/platform/accessibility-standards.ts`
  - minimum target is `WCAG 2.1 AA`
  - desktop-only workflows are still bound by keyboard, focus, contrast, dialog, and semantic requirements
- **Performance standards:** `apps/web/lib/platform/performance-standards.ts`
  - defines p50/p95 latency budgets for user-facing reads
  - defines target/max runtime expectations for long-running jobs
  - sets `5s` as the threshold for moving work into an explicit background-job/status flow
- **Observability standards:** `apps/web/lib/platform/observability-standards.ts`
  - current stack is structured logging + targeted telemetry + product analytics primitives
  - alert ownership is assigned across platform, creator, growth, media-processing, and billing domains
...
6. **Observability vendor integrations**: Layer dedicated external collectors on top of the current structured-logging and telemetry baseline
